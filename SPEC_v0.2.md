...
|---|---|---|---|
| **P1** | 운영 효율 | MCC 통합 / 광고 구조 CRUD / 다중 선택 일괄 작업 / CSV 업로드 / 변경 프리뷰·롤백 / 기본 대시보드 / 이상 징후 알림 | 6~8주 |
...
### 5.1 MCC · 광고주
- **MasterAccount**: 1개(또는 소수). MCC 마스터 계정의 API 키·시크릿 보관
- **Advertiser**: MCC 산하 광고주 N개. `customerId` 기준. ManagedCustomerLink로 동기화
...
| ID | 기능 | 요구사항 |
|---|---|---|
| F-1.1 | 마스터 계정 등록 | CUSTOMER_ID·API_KEY·SECRET_KEY 등록. SECRET 암호화 저장. |
| F-1.2 | 광고주 목록 동기화 | `/customer-links?type=MYCLIENTS` 1시간 주기. ManagedCustomerLink |
| F-1.3 | 광고주 메타정보 | 표시명·카테고리·담당자·메모·태그 |
| F-1.4 | 광고주 셀렉터 | GNB 상단 셀렉터로 1개 선택. 컨텍스트 전환 |
| F-1.5 | 연결 상태 점검 | 헬스체크 + 비즈머니 잔액 표시 |
...
- **SECRET 암호화 방식**:
  ...
  - DB 컬럼: `secretKeyEnc` (암호문) + `secretKeyVersion` (키 버전, 정수). 향후 rotation 시 신키로 재암호화하며 버전 증가
...
| 엔티티 | 주요 필드 | 비고 |
|---|---|---|
| MasterAccount | id, name, customerId, apiKeyEnc, secretKeyEnc, status | MCC 마스터 |
| Advertiser | id, masterId, customerId, name, bizNo, category, manager, tags, status | 광고주 |
...
| 대상 | 용도 | 비고 |
|---|---|---|
| 네이버 SA API | 광고 CRUD · 성과 · 확장소재 · Estimate · StatReport | HMAC-SHA256, X-Customer 헤더로 광고주 전환. **Rate Limit 사전 협의 필수** |
...
- **LNB**: 대시보드 / 캠페인 / 광고그룹 / 키워드 / 소재 / 확장소재 / 알림 / (P2: 비딩 최적화) / 설정
...
- [ ] MCC 마스터 계정 발급 상태 (API 신청·승인·키 발급)
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| **네이버 SA Rate Limit 불투명** | 1만 키워드 일괄 작업 실패·지연 | 사전 협의 필수. NaverSAClient 토큰 버킷 큐. 일괄 PUT 청크 분할 |
