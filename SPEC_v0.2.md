...
### v0.2.2 → v0.2.3 변경 요약 (BidSuggestion 묶음 권고 구조)
- **BidSuggestion 묶음 권고 구조 (scope/affectedCount/targetName/itemsJson) 도입**: 키워드 1:1 권고만 가능하던 구조를 광고그룹·캠페인 단위 묶음으로 확장. 5천 행 단위 권고 폭발 → Inbox 운영 불가능 문제 해결. 검색어 클러스터는 ApprovalQueue 로 별도 분리
...
| Campaign | id, advertiserId, nccCampaignId, name, type, dailyBudget, status, **statusReason**, raw | statusReason: SA 응답 OFF 사유 (예: "그룹 OFF", "캠페인 예산 도달"). ON 행은 null. UI 배지 옆 인라인 표시 |
| AdGroup | id, campaignId, nccAdgroupId, name, bidAmt, dailyBudget, status, **statusReason**, raw | statusReason: 동일 패턴 (예: "캠페인 OFF") |
| Keyword | id, adgroupId, nccKeywordId, keyword, bidAmt, useGroupBidAmt, status, **statusReason**, raw | statusReason: 동일 패턴 |
...
| BidSuggestion (P2) | id, advertiserId, keywordId?, adgroupId?, engineSource, action, reason, severity, status, expiresAt, appliedBatchId?, **scope(keyword/adgroup/campaign), affectedCount, targetName?, itemsJson?**, createdAt, updatedAt | Inbox 권고 (5종 엔진 적재). 묶음 권고: `scope ≠ 'keyword'` 시 itemsJson에 N개 키워드 변경 표현 (`[{ keywordId, beforeBid, afterBid, reason? }]`). affectedCount는 items.length 캐시. targetName은 광고그룹/캠페인 표시명 redundant 캐시 |
