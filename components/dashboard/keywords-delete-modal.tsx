...
/**
 * 키워드 단건 삭제 모달 (F-3.7) — admin 한정 + 2차 확인
 *
 * 흐름 (3단계 상태 머신):
 *   1. confirm     — 삭제 대상 정보 노출 + 키워드 텍스트 재입력 칸
 *                   trim 비교 일치 시에만 삭제 버튼 활성. 빈 입력/불일치 → disabled.
 *                   "삭제(destructive)" → submitting
 *   2. submitting  — 스피너 + "삭제 중...". deleteKeywordSingle 호출
 *                   catch:
 ...
 *
 * 폼 라이브러리:
 *   useState — confirmKeyword 단일 입력 + 동기 비교만. 다른 모달 (keywords-add-modal)
 *   과 동일 패턴 유지.
 *
 * SPEC 6.2 F-3.7 / 안전장치 6 (대량 삭제 비대상 + 단건 삭제 admin + 2차 확인).
 */
...
/** 모달 진입에 필요한 최소 row 정보. KeywordRow 의존성 차단 위해 별도 타입. */
...
export function KeywordsDeleteModal({
  advertiserId,
  row,
  open,
  onOpenChange,
  ...
  const [confirmKeyword, setConfirmKeyword] = React.useState<string>("")
  const [serverMismatch, setServerMismatch] = React.useState<boolean>(false)
  const [result, setResult] = React.useState<DeleteKeywordResult | null>(null)

  // -- 2차 확인 검증 ----------------------------------------------------------
  const trimmedInput = confirmKeyword.trim()
  const trimmedTarget = row.keyword.trim()
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (msg === "확인 키워드 텍스트 불일치") {
        // race condition (사용자가 입력 중 키워드 변경되었거나 백엔드 검증 시점 차이).
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "confirm" &&
              "이 작업은 되돌릴 수 없습니다 (P1 비대상). 삭제할 키워드의 텍스트를 다시 입력해 확인하세요."}
            {step === "submitting" && "키워드를 삭제하고 있습니다..."}
            {step === "result" &&
              (result?.ok
                ? "키워드 삭제 결과를 확인하세요."
                : "삭제에 실패했습니다.")}
...
function ConfirmView({
  ...
  setConfirmKeyword: (v: string) => void
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="delete-confirmKeyword" className="text-sm">
          확인을 위해 키워드 텍스트를 다시 입력하세요
        </Label>
        <Input
          id="delete-confirmKeyword"
        ...
        {showMismatchError && (
          <p className="text-[11px] text-destructive">
            텍스트가 일치하지 않습니다. 위에 표시된 키워드와 정확히 동일하게
            입력하세요.
          </p>
        )}
      </div>
...
function ResultView({ result }: { result: DeleteKeywordResult }) {
  if (!result.ok) {
    return (
      <div className="rounded-md border border-destructive/40 bg-destructive/5 px-3 py-3 text-sm text-destructive">
        <div className="font-medium">키워드 삭제 실패</div>
        <div className="mt-1 text-xs">{result.error}</div>
      </div>
    )
  }
  return <SuccessView batchId={result.batchId} nccKeywordId={result.nccKeywordId} />
}
...
function SuccessView({
  ...
  // batchId="" 인 idempotent 케이스 (이미 deleted 였던 키워드) 분기 처리.
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-900/40 dark:bg-emerald-900/10 dark:text-emerald-300">
        <div className="font-medium">키워드 삭제 완료</div>
