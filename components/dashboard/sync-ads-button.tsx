 ...
 * 소재 동기화 버튼 (F-4.1)
 *
 * - 광고주의 모든 광고그룹을 순회하며 SA listAds → DB upsert
 * - server action `syncAds(advertiserId)` 호출
 *   응답: { ok: true; syncedAds; scannedAdgroups; skipped; durationMs }
 ...
 * - 결과 toast: "소재 N개 동기화 완료 (M개 그룹 / S건 스킵 / X.Xs)"
 *   · skipped 는 "광고그룹 매핑 누락 등으로 적재되지 않은 소재 수" — 사용자 인지 필요
 * - 성공 시 `router.refresh()` 로 RSC 재조회
 *
 * sync-keywords-button.tsx 와 동일 패턴.
 *
 * SPEC 6.2 F-4.1.
 */
...
export function SyncAdsButton({
  ...
  function handleClick() {
    ...
    toast.promise(
      (async () => {
        const res = await syncAds(advertiserId)
