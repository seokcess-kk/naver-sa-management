 ...
 * 소재 동기화 + 캠페인 필터 (F-4.1)
 *
 * sync-extensions-with-filter.tsx 패턴 복제. server action `syncAds(advertiserId, options?)` 호출.
 *   options: { campaignIds?: string[] }
 *
 * 동작:
 * - 캠페인 0개 선택 → 광고주 전체 동기화 (기본 동작 유지)
 * - 캠페인 N개 선택 → `syncAds(advertiserId, { campaignIds })` 로 부분 동기화
 * - 응답: { ok: true; syncedAds; scannedAdgroups; skipped; durationMs } | { ok: false; error }
 *
 * 키 미설정(`hasKeys=false`) → 비활성화 + 안내 tooltip (sync-ads-button 동일).
 *
 ...
 *
 * 본 컴포넌트는 AdsPage 만 사용. sync-ads-button.tsx 는 무변경 (다른 페이지 영향 X).
 *
 * SPEC 6.2 F-4.1 / 11.2.
 */
...
import { syncAds } from "@/app/(dashboard)/[advertiserId]/ads/actions"
...
export type SyncAdsWithFilterProps = {
  advertiserId: string
  hasKeys: boolean
...
export function SyncAdsWithFilter({
  advertiserId,
  hasKeys,
  campaigns,
...
function SyncAdsWithFilterInner({
  hasKeys,
  running,
  campaigns,
  selectedIds,
        ...
        <Dialog>
          ...
          <DialogContent className="max-w-sm">
            <DialogHeader>
              ...
              <DialogDescription>
                선택한 캠페인 산하 광고그룹의 소재만 동기화합니다. 미선택 시
                광고주 전체.
              </DialogDescription>
            </DialogHeader>
            <CampaignMultiSelect
